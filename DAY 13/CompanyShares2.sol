...
contract CompanyShares {
    ...
    function buyShares(uint _amount) 
        payable public isParticipantAllowedToBuy 
        validateNoNegative(_amount) validateAdditionEdgeCase(_amount) returns (uint) {
    ...
    function withdrawFunds(uint _amount) 
        public isParticipantAllowedToWithdraw 
        validateNoNegative(_amount) validateAdditionEdgeCase(_amount) returns (uint) {
